...
function withCustomNativeModules(config) {
  ...
  config = withDangerousMod(config, [
    ...
    async cfg => {
      ...
      const sourceDir = path.join(projectRoot, 'native-src', 'android', 'com', 'vega');
      ...
      if (fs.existsSync(sourceDir)) {
        ...
        for (const file of files) {
          if (file.endsWith('.kt')) {
            ...
            content = content.replace(/^package com\.vega$/m, `package ${packageName}`);
  ...
  config = withMainApplication(config, cfg => {
    ...
    const packagesToAdd = ['DohPackage()', 'TorrentPackage()'];
